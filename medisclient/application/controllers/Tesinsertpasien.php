...
class Tesinsertpasien extends CI_Controller {
	...
	function index(){
		$data['action'] = 'http://localhost/medisclient/index.php/tesinsertpasien';
		...
		// pengecekan error
			...
			if ($error) {
				...
				$param = array('nama' => $post['nama'], 'jeniskelamin' => $post['jeniskelamin'], 'nik' => $post['nik'], 'notlp' => $post['notlp'], 'alamat' => $post['alamat'], 'tanggallahir' => $post['tanggallahir'], 'golongandarah' => $post['golongandarah']);
				
				// memanggil dan mengeksekusi metode/fungsi penjumlahan di server
				$result = $this->nusoap_client->call('insertpasien',$param);
				var_dump($result);
		...
		$this->load->view('tesinsertpasien', $data);
	}
}
?>